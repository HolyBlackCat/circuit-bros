...
#define IMPL_ADJUST(capture, object, ...) (void(), [capture]{ auto _ = object; MA_VA_FOR_EACH(,IMPL_ADJUST_STEP,MA_TR_C(__VA_ARGS__)) return _; }())
#define IMPL_ADJUST_STEP(data, i, expr) _.expr;

#define IMPL_WITH(capture, ...) ->* ::Macro::WithExpr([capture](auto &_) { MA_VA_FOR_EACH(,IMPL_ADJUST_STEP,MA_TR_C(__VA_ARGS__)) })
